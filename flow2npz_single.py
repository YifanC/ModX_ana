...
def flow2npz(infile, outfile):

    f_manager = h5flow.data.H5FlowDataManager(infile, 'r')
    ...
    for i_evt in f_n_evt['id']:
        ...
        if i_evt % 1E4 == 0:    
            arr_edge_xmax_hits = np.array(edge_xmax_hits)
            arr_edge_xmin_hits = np.array(edge_xmin_hits)
            arr_edge_ymax_hits = np.array(edge_ymax_hits)
            arr_edge_ymin_hits = np.array(edge_ymin_hits)
            arr_edge_zmax_hits = np.array(edge_zmax_hits)
            arr_edge_zmin_hits = np.array(edge_zmin_hits)
            arr_edge_cathode_xplus_hits = np.array(edge_cathode_xplus_hits)
            arr_edge_cathode_xminus_hits = np.array(edge_cathode_xminus_hits)

            arr_evt_crossing_track = np.array(evt_crossing_track)

            arr_ct_fit_length = np.array(ct_fit_length)
            arr_ct_PCA_mean = np.array(ct_PCA_mean)
            arr_ct_PCA_vec = np.array(ct_PCA_vec)
            arr_ct_PCA_variance = np.array(ct_PCA_variance)
            arr_ct_PCA_singular_values = np.array(ct_PCA_singular_values)
            arr_ct_pts_density = np.array(ct_pts_density)

            arr_seg_fit_length = np.array(seg_fit_length)
            arr_seg_PCA_mean = np.array(seg_PCA_mean)
            arr_seg_PCA_vec = np.array(seg_PCA_vec)
            arr_seg_PCA_variance = np.array(seg_PCA_variance)
            arr_seg_PCA_singular_values = np.array(seg_PCA_singular_values)
            arr_seg_pts_density = np.array(seg_pts_density)

            arr_seg_cos_angle = np.array(seg_cos_angle)
            arr_seg_angle = np.array(seg_angle)
            arr_seg_joint = np.array(seg_joint)

            arr_ct_ends = np.array(ct_ends)
            
            np.savez(outfile,
                edge_xmax_hits = arr_edge_xmax_hits,
                edge_xmin_hits = arr_edge_xmin_hits,
                edge_ymax_hits = arr_edge_ymax_hits,
                edge_ymin_hits = arr_edge_ymin_hits,
                edge_zmax_hits = arr_edge_zmax_hits,
                edge_zmin_hits = arr_edge_zmin_hits,
                edge_cathode_xplus_hits = arr_edge_cathode_xplus_hits,
                edge_cathode_xminus_hits = arr_edge_cathode_xminus_hits,
                evt_crossing_track = arr_evt_crossing_track,
                ct_fit_length = arr_ct_fit_length,
                ct_PCA_mean = arr_ct_PCA_mean,
                ct_PCA_vec = arr_ct_PCA_vec,
                ct_PCA_variance = arr_ct_PCA_variance,
                ct_PCA_singular_values = arr_ct_PCA_singular_values,
                ct_pts_density = arr_ct_pts_density,
                seg_fit_length = arr_seg_fit_length,
                seg_PCA_mean = arr_seg_PCA_mean,
                seg_PCA_vec = arr_seg_PCA_vec,
                seg_PCA_variance = arr_seg_PCA_variance,
                seg_PCA_singular_values = arr_seg_PCA_singular_values,
                seg_pts_density = arr_seg_pts_density,
                seg_cos_angle = arr_seg_cos_angle,
                seg_angle = arr_seg_angle,
                seg_joint = arr_seg_joint,
                ct_ends = arr_ct_ends)
